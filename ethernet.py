...
# ===== estado global =====
EthernetProtocols: Dict[int, Callable] = {}
macAddress: Optional[bytes] = None
handle: Optional[pcap_t] = None
levelInitialized: bool = False
recvThread = None   # tipo rxThread, pero así evitamos líos de tipos
...
def process_Ethernet_frame(us:ctypes.c_void_p,header:pcap_pkthdr,data:bytes) -> None:
    ...
    if dst != macAddress and dst != broadcastAddr:
        return  
    #Callback registrado
    cb = EthernetProtocols.get(ethertype)
    if cb is None :
        return    
    try:
        cb(us, header, payload, src)
    except Exception as e:
        logging.debug(f"Callback error {ethertype:04X} lanzó excepción: {e}")
        return -1
...
def startEthernetLevel(interface:str) -> int:
    ...
    try: 
        ...
        logging.error("No se pudo obtener la MAC de la interfaz {interface}:{e}")
        return  -1
    
    errbuf = bytearray(256)
    handle = pcap_open_live(interface, ETH_FRAME_MAX, 1, TO_MS, errbuf)
    if handle is None:
        logging.error(f"pcap_open_live falló {errbuf.decode(errors = 'ignore')}")
        return -1
...
def stopEthernetLevel()->int:
    ...
    if not levelInitialized:
        logging.error('Ethernet no inicializada')
        return -1
...
def sendEthernetFrame(data:bytes,length:int,etherType:int,dstMac:bytes) -> int:
    ...
    if not levelInitialized or handle is None or macAddress is None:
        logging.error('Ethernet no inicializado')
        return -1
    if length != len(data):  #Por si la longitud no coincide
        data = data[:length]
    frame = dstMac + macAddress + struct.pack ('!H',etherType) + data
    
    if len(frame)<ETH_FRAME_MIN :
        frame = frame.ljust(ETH_FRAME_MIN, b'\x00')
    if len(frame)>ETH_FRAME_MAX : 
        logging.error('La trama es muy grande')
        return -1

    sent = pcap_inject(handle, frame, len(frame))
    if sent < 0 :
        logging.error('Fallo en pcap_inject')
        return -1
    return 0 
